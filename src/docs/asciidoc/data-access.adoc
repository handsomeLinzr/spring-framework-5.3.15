...
* <<tx-resources-synchronization, Synchronizing resources with transactions>> describes
...
Local transactions are resources-specific, such as a transaction associated with a JDBC
...
single transaction resources.) Another downside is that local transactions are invasive
...
can enlist any transactional resources.
...
[[tx-resources-synchronization-high]]
...
proxies for managing the native resources factories. These transaction-aware solutions
internally handle resources creation and reuse, cleanup, optional transaction
...
[[tx-resources-synchronization-low]]
...
application code to deal directly with the resources types of the native persistence APIs,
...
[[tx-resources-synchronization-tadsp]]
...
the underlying resources transactions are different and, hence, can commit or roll back
...
is typically mapped onto JDBC savepoints, so it works only with JDBC resources
...
Any DAO or repository implementation needs access to a persistence resources,
...
`EntityManager`. The easiest way to accomplish this is to have this resources dependency
...
own new code by using the higher level abstractions for resources management, such as
...
You can use this implementation instead of `JtaTransactionManager` in the single-resources
...
supports native Hibernate for resources management, data access object (DAO) implementations,
...
tools through Dependency Injection. They can participate in Spring's resources and
...
* *General resources management.* Spring application contexts can handle the location
...
* *Integrated transaction management.* You can wrap your ORM code with a declarative,
  ...
  (rollback and so on) are handled for you. As discussed in <<orm-resources-mngmnt>>,
...
hard-coded resources lookups, no more hard-to-replace singletons, no more custom service
...
[[orm-resources-mngmnt]]
==== Resource and Transaction Management

Typical business applications are cluttered with repetitive resources management code.
...
resources handling, namely IoC through templating in the case of JDBC and applying AOP
interceptors for the ORM technologies.

The infrastructure provides proper resources handling and appropriate conversion of
...
To avoid tying application objects to hard-coded resources lookups, you can define
...
resources definitions in the container or locally within the application is mainly a
...
Especially in the typical case of single-database transactions, Spring's single-resources
...
requires more work than deploying a simple web application with local resources
...
* Spring is aware that Hibernate itself is synchronized to the JTA transaction and
  ...
  Hibernate's transactional resources management.
* The JTA transaction commits.
...
On the Java EE platform, they are used for dependency declaration and not for resources
injection.
****
...
JDBC transactions (such as transaction-specific isolation levels and resources-level
...
As an alternative to `JpaTransactionManager`, Spring also allows for multi-resources
...
`jaxb.properties` file, and possibly some resources files. JAXB also offers a way to
