...
facility to transparently store a resources alongside a thread. Setting up a
...
`ThreadLocal.set(null)`) the resources local to the thread. Unsetting should be done in
