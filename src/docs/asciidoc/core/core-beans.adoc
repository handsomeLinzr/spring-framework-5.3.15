...
supplied to an `ApplicationContext` constructor are resources strings that let
...
----
	<beans>
		<import resources="services.xml"/>
		<import resources="resources/messageSource.xml"/>
		<import resources="/resources/themeSource.xml"/>
...
You can always use fully qualified resources locations instead of relative paths: for
...
You may want to do that by default for a resources that you acquire with JNDI, as its
...
intend to refer to the provided resources here).
=====
...
Any `${...}` placeholders present in a `@PropertySource` resources location are
...
----
	<beans>
		<import resources="com/bank/service/${customer}-config.xml"/>
	</beans>
----
...
The example assumes that you have three resources bundles called `format`, `exceptions` and `windows`
...
purposes of the example, assume the contents of two of the above resources bundle files
are as follows:
...
number of resources bundles through its `basenames` property. The three files that are
...
files from any Spring resources location (not only from the classpath) and supports hot
...
describable with a standard URL, and some other variations. If the resources location
...
resources strings and, in simple form, are treated appropriately according to the specific
context implementation. For example `ClassPathXmlApplicationContext` treats a simple
location path as a classpath location. You can also use location paths (resources strings)
