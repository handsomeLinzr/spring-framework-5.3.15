...
resources that needs to be obtained from the classpath or relative to a
...
such as a method to check for the existence of the resources being pointed to.
...
* `getInputStream()`: Locates and opens the resources, returning an `InputStream` for
  reading from the resources. It is expected that each invocation returns a fresh
  `InputStream`. It is the responsibility of the caller to close the stream.
* `exists()`: Returns a `boolean` indicating whether this resources actually exists in
  physical form.
* `isOpen()`: Returns a `boolean` indicating whether this resources represents a handle
  with an open stream. If `true`, the `InputStream` cannot be read multiple times and
  must be read once only and then closed to avoid resources leaks. Returns `false` for
  all usual resources implementations, with the exception of `InputStreamResource`.
* `getDescription()`: Returns a description for this resources, to be used for error
  output when working with the resources. This is often the fully qualified file name or
  the actual URL of the resources.

Other methods let you obtain an actual `URL` or `File` object representing the
resources (if the underlying implementation is compatible and supports that
functionality).
...
for a resources that supports writing to it.

Spring itself uses the `Resource` abstraction extensively, as an argument type in
many method signatures when a resources is needed. Other methods in some Spring APIs
...
This class represents a resources that should be obtained from the classpath. It uses
...
resources resides in the file system but not for classpath resources that reside in a
...
when the web application archive is expanded and the resources is physically on the
...
already-opened resources. Therefore, it returns `true` from `isOpen()`. Do not use it if
you need to keep the resources descriptor somewhere or if you need to read a stream
multiple times.
...
.Java
----
	Resource template = ctx.getResource("some/resources/path/myTemplate.txt");
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	val template = ctx.getResource("some/resources/path/myTemplate.txt")
----
...
.Java
----
	Resource template = ctx.getResource("classpath:some/resources/path/myTemplate.txt");
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	val template = ctx.getResource("classpath:some/resources/path/myTemplate.txt")
----
...
.Java
----
	Resource template = ctx.getResource("file:///some/resources/path/myTemplate.txt");
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	val template = ctx.getResource("file:///some/resources/path/myTemplate.txt")
----

[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
	Resource template = ctx.getResource("https://myhost.com/resources/path/myTemplate.txt");
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	val template = ctx.getResource("https://myhost.com/resources/path/myTemplate.txt")
----

The following table summarizes the strategy for converting `String` objects to `Resource`
objects:

[[resources-resources-strings]]
.Resource strings
|===
...
As can be seen above, this interface also defines a special `classpath*:` resources prefix
for all matching resources from the class path. Note that the resources location is
...
<<resources-app-ctx-wildcards-in-resources-paths>> and its subsections for further details
on wildcard support with the `classpath*:` resources prefix.
...
resolve a specified resources location path into one or more matching `Resource` objects.
...
interface if that is all you need. The code would be coupled only to the resources loading
...
NOTE: To load one or more `Resource` objects for a resources path that contains wildcards
or makes use of the special `classpath*:` resources prefix, consider having an instance of
...
If the bean itself is going to determine and supply the resources path through some sort
...
of a template of some sort, where the specific resources that is needed depends on the
...
string for that resources, as the following example shows:

[source,xml,indent=0,subs="verbatim,quotes"]
----
	<bean id="myBean" class="example.MyBean">
		<property name="template" value="some/resources/path/myTemplate.txt"/>
	</bean>
----

Note that the resources path has no prefix. Consequently, because the application context
itself is going to be used as the `ResourceLoader`, the resources is loaded through a
...
[source,xml,indent=0,subs="verbatim,quotes"]
----
	<property name="template" value="classpath:some/resources/path/myTemplate.txt">
----

[source,xml,indent=0,subs="verbatim,quotes"]
----
	<property name="template" value="file:///some/resources/path/myTemplate.txt"/>
----
...
[[resources-app-ctx-wildcards-in-resources-paths]]
=== Wildcards in Application Context Constructor Resource Paths

The resources paths in application context constructor values may be simple paths (as
...
Note that this wildcarding is specific to the use of resources paths in application context
...
a resources points to just one resources at a time.
...
it is always a `java.io.File` representing the directory (where the classpath resources
resolves to a filesystem location) or a jar URL of some sort (where the classpath resources
...
class loader hierarchy and then, off each resources, the same `PathMatcher` resolution
...
Consider the following example of a resources location:
...
Such a resources may exist in only one location in the classpath, but when a path such as
...
multiple `ClassLoader` locations, the desired resources may not exist in the first
...
----
	...
	ctx.getResource("some/resources/path/myTemplate.txt");
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
...
----
	...
	ctx.getResource("file:///some/resources/path/myTemplate.txt");
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
