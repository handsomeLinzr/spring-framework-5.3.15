...
Test classes typically declare either an array of resources locations for XML or Groovy
...
`@ContextConfiguration` declares the application context resources `locations` or the
...
resources locations can also refer to files and scripts in the file system, and component
...
As an alternative or in addition to declaring resources locations or component classes,
...
since the default loader supports `initializers` and either resources `locations` or
component `classes`.
...
NOTE: `@ContextConfiguration` provides support for inheriting resources locations or
...
resources base path). The resources base path is used behind the scenes to create a
...
To override the default, you can specify a different base resources path by using the
implicit `value` attribute. Both `classpath:` and `file:` resources prefixes are
supported. If no resources prefix is supplied, the path is assumed to be a file system
resources. The following example shows how to specify a classpath resources:
...
----
<1> Specifying a classpath resources.
...
are managed by an external resources such as for a container managed by the
...
process resources locations, component classes, or context initializers. Furthermore, a
...
* `AnnotationConfigWebContextLoader`: Loads a `WebApplicationContext` from component
  classes.
* `GenericGroovyXmlContextLoader`: Loads a standard `ApplicationContext` from resources
  locations that are either Groovy scripts or XML configuration files.
* `GenericGroovyXmlWebContextLoader`: Loads a `WebApplicationContext` from resources
  locations that are either Groovy scripts or XML configuration files.
* `GenericXmlContextLoader`: Loads a standard `ApplicationContext` from XML resources
  locations.
* `GenericXmlWebContextLoader`: Loads a `WebApplicationContext` from XML resources
  locations.
...
class level. If your test class does not explicitly declare application context resources
...
resources locations and component classes, an application context can also be configured
...
an array that contains the resources locations of XML configuration metadata. A plain or
relative path (for example, `context.xml`) is treated as a classpath resources that is
...
path that represents a resources URL (i.e., a path prefixed with `classpath:`, `file:`,
...
attribute name and declare the resources locations by using the shorthand format
...
XML resources location. Specifically, `GenericXmlContextLoader` and
...
attribute with an array that contains the resources locations of Groovy scripts. Resource
...
configured resources location ends with `.xml`, it is loaded by using an
...
the Spring Framework delivers in the `spring-test` module support only one resources type
...
If you want to use resources locations (for example, XML or Groovy) and `@Configuration`
...
define either a set of XML or Groovy resources locations or a set of `@Configuration`
...
attributes that denote whether resources locations or component classes and context
initializers declared by superclasses should be inherited. The default value for both
flags is `true`. This means that a test class inherits the resources locations or
component classes as well as the context initializers declared by any superclasses.
Specifically, the resources locations or component classes for a test class are appended
to the list of resources locations or annotated classes declared by superclasses.
Similarly, the initializers for a given test class are added to the set of initializers
defined by test superclasses. Thus, subclasses have the option of extending the resources
...
is set to `false`, the resources locations or component classes and the context
...
In the next example, which uses XML resources locations, the `ApplicationContext` for
...
class to declare resources locations for test properties files or inlined properties.
...
`"test.properties"`) is treated as a classpath resources that is relative to the package
in which the test class is defined. A path starting with a slash is treated as an
absolute classpath resources (for example: `"/org/example/test.xml"`). A path that
references a URL (for example, a path prefixed with `classpath:`, `file:`, or `http:`) is
loaded by using the specified resources protocol. Resource location wildcards (such as
`**/*.properties`) are not permitted: Each location must evaluate to exactly one
`.properties` or `.xml` resources.
...
from resources locations. Note, however, that properties registered via
...
attributes that denote whether resources locations for properties files and inlined
...
external resources whose lifecycle is maintained outside the test's `ApplicationContext`.
====
...
created and supplied to your test's WAC. By default, the base resources path for your
...
reference a base resources path from the classpath instead of the file system, you can use
...
If you annotate a test class with `@WebAppConfiguration` without specifying a resources
base path, the resources path effectively defaults to `file:src/main/webapp`. Similarly,
if you declare `@ContextConfiguration` without specifying resources `locations`, component
...
The following example shows how to explicitly declare a resources base path with
`@WebAppConfiguration` and an XML resources location with `@ContextConfiguration`:

.Default resources semantics
[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
	@ExtendWith(SpringExtension.class)
...
annotations. By default, `@WebAppConfiguration` resources paths are file system based,
whereas `@ContextConfiguration` resources locations are classpath based.

The following example shows that we can override the default resources semantics for both
annotations by specifying a Spring resources prefix:

.Explicit resources semantics
[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
	@ExtendWith(SpringExtension.class)
...
a `ServletWebRequest` based on the base resources path configured with
...
given level in the hierarchy, the configuration resources type (that is, XML configuration
files or component classes) must be consistent. Otherwise, it is perfectly acceptable to
have different levels in a context hierarchy configured using different resources types.
...
configure individual SQL statements or the resources paths to SQL scripts that should be
...
Each path is interpreted as a Spring `Resource`. A plain path (for example,
`"schema.sql"`) is treated as a classpath resources that is relative to the package in
which the test class is defined. A path starting with a slash is treated as an absolute
classpath resources (for example, `"/org/example/schema.sql"`). A path that references a
URL (for example, a path prefixed with `classpath:`, `file:`, `http:`) is loaded by using
the specified resources protocol.
