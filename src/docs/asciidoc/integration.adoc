...
| `headForHeaders`
| Retrieves all headers for a resources by using HEAD.

| `postForLocation`
| Creates a new resources by using POST and returns the `Location` header from the response.

| `postForObject`
| Creates a new resources by using POST and returns the representation from the response.

| `postForEntity`
| Creates a new resources by using POST and returns the representation from the response.

| `put`
| Creates or updates a resources by using PUT.

| `patchForObject`
| Updates a resources by using PATCH and returns the representation from the response.
...
| `optionsForAllow`
| Retrieves allowed HTTP methods for a resources by using ALLOW.
...
know that now you have to create a Spring container configuration resources named
...
resources into Spring's transaction management mechanisms.
...
typically require that you declare a JMS connection factory as a `resources-ref` inside
...
objects are created and destroyed. To optimize the resources usage and increase
...
transactions, resources acquisition and release, exception conversion, and so on. This
...
The `JmsTransactionManager` performs local resources transactions, binding a JMS
...
You can activate local resources transactions through the `sessionTransacted` flag
...
part of the same local transaction, but any other resources operations (such as
...
It uses the same underlying resources provider contract. As with EJB 2.1 MDBs, you can use any
...
`destinationResolver`) as well as basic JMS settings and resources references. By using
...
[source,xml,indent=0,subs="verbatim,quotes"]
----
	<jms:jca-listener-container resources-adapter="myResourceAdapter"
			destination-resolver="myDestinationResolver"
			transaction-manager="myTransactionManager"
			concurrency="10">
...
| `ObjectName`
| Used by `MetadataNamingStrategy` to determine the `ObjectName` of a managed resources.
| `ManagedResource`

| `description`
| Sets the friendly description of the resources, attribute or operation.
...
also hides the JMX-managed resources annotations. Hence, you should use target-class proxies in that
...
low-level resources handling on behalf of the client.
...
and the resources are very important. Be sure to first add the text and then
