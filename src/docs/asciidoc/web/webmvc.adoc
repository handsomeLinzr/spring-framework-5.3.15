...
name (such as `redirect:/myapp/some/resources`) redirects relative to the current
...
technology but still want to force a forward of a resources to be handled by the
...
representation of the current resources regardless of the logical view name. The `Accept`
...
resources bundle loading mechanism, allowing for full internationalization of themes. For
...
| Servlet 4.0 push builder API for programmatic HTTP/2 resources pushes.
...
intended recipient but another asynchronous request (for example, polling or resources requests),
...
`lastModified` or `ETag` value for a resources needs to be calculated before it can be compared
...
The resources handler also supports a chain of
{api-spring-framework}/web/servlet/resources/ResourceResolver.html[`ResourceResolver`] implementations and
{api-spring-framework}/web/servlet/resources/ResourceTransformer.html[`ResourceTransformer`] implementations,
which you can use to create a toolchain for working with optimized resources.

You can use the `VersionResourceResolver` for versioned resources URLs based on an MD5 hash
...
<mvc:resources mapping="/resources/**" location="/public/">
	<mvc:resources-chain resources-cache="true">
		<mvc:resolvers>
...
for fine-grained control, e.g. last-modified behavior and optimized resources resolution.
...
of the container's default Servlet), while still allowing static resources requests to be
