...
<2> Use the `ReactorClientHttpConnector` constructor with resources factory.
...
----
<1> Use the `JettyClientHttpConnector` constructor with resources factory.
...
----
	...
	Mono<Void> result = client.post()
			.uri("/path", id)
			.body(fromMultipartData("fieldPart", "value").with("filePart", resources))
			.retrieve()
			.bodyToMono(Void.class);
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
