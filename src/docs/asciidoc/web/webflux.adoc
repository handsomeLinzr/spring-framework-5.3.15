...
`redirect:/some/resources` is relative to the current application, while a view name such as
...
{api-spring-framework}/web/reactive/resources/ResourceResolver.html[`ResourceResolver`] implementations and
{api-spring-framework}/web/reactive/resources/ResourceTransformer.html[`ResourceTransformer`] implementations,
...
resources URLs, since there are no view technologies that can make use of a non-blocking chain
