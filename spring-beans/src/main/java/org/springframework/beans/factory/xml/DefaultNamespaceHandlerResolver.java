...
public class DefaultNamespaceHandlerResolver implements NamespaceHandlerResolver {
	...
	// handler文件的路径，在构造函数DefaultNamespaceHandlerResolver的时候已经传进来了 META-INF/spring.handlers
	...
	public DefaultNamespaceHandlerResolver(@Nullable ClassLoader classLoader) {
		// DEFAULT_HANDLER_MAPPINGS_LOCATION = META-INF/spring.handlers，handler文件的路径，
		...
		if (handlerMappings == null) {
			...
			synchronized (this) {
				...
				if (handlerMappings == null) {
					...
					try {
						// 加载文件 META-INF/spring.handlers，加载成 Properties 对象
						// 这里的加载，并不单单是只加载一个文件，是扫描整个引入的包中，所有位于 META-INF/spring.handlers 的文件
						Properties mappings =
