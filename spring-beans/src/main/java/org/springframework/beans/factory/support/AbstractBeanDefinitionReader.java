...
public abstract class AbstractBeanDefinitionReader implements BeanDefinitionReader, EnvironmentCapable {
	...
	// 在构造函数的时候设置，设置的是 beanFactory
	// 调用 loadBeanDefinitions 的时候，调用了 new XmlBeanDefinitionReader(beanFactory) 的时候创建了xml的bd解析器
	...
	// 默认没其他地方调用，则为null，后边用到了会自动取到并设置为 Thread.currentThread.getClassLoader==appClassLoader
	@Nullable
	private ClassLoader beanClassLoader;

	// applicationContext 的 env
	...
	// 又是重载方法 loadBeanDefinitions，通过资源对象加载bd
	@Override
	public int loadBeanDefinitions(Resource... resources) throws BeanDefinitionStoreException {
		Assert.notNull(resources, "Resource array must not be null");
		int count = 0;
		for (Resource resource : resources) {
			// 1.重载 loadBeanDefinitions，加载资源对象
			// 2.加载的bd数量统计++
	...
	public int loadBeanDefinitions(String location, @Nullable Set<Resource> actualResources) throws BeanDefinitionStoreException {
		// getResourceLoader 返回当前reader的资源加载器
		// 在localBeanDefinitions最开始的时候，刚创建好reader时，后边调用了 setResourceLoader(this)，传进去了applicationContext容器
		// 所以这里的资源加载器其实就是当前的 applicationContext 对象
		...
		if (resourceLoader instanceof ResourcePatternResolver) {
			...
			// 默认，当前 ClasspathXmlApplicationContext 属于 ResourcePatternResolver，所以会进入这个分支中
			try {
				// 获取当前路径对应的资源对象，这里返回一个资源数组，实际上只有一个资源对象
	...
	public int loadBeanDefinitions(String... locations) throws BeanDefinitionStoreException {
		...
		for (String location : locations) {
			// 遍历所有的配置的配置，调用冲在的 loadBeanDefinitions 方法进行解析
			// 每解析一个配置文件，count++
