...
public class StandardEnvironment extends AbstractEnvironment {
	...
	public StandardEnvironment() {
		// 1.StandardEnvironment 继承 AbstractEnvironment，先调用 AbstractEnvironment 构造函数
		// 2.AbstractEnvironment 构造函数会调用到 customizePropertySources，回调了子类这里的方法
	}
	 ...
	 *
	 * 父类 AbstractEnvironment 的构造函数会调用到这个方法，所以在new的时候会触发这个方法的调用
	 *
	 */
	@Override
	protected void customizePropertySources(MutablePropertySources propertySources) {
		// 记录系统变量到environment，都是从 System 中获取
		// System.getProperties() -> systemProperties
